...
# 1. 정답지(CSV) 불러오기 (../../ 를 붙여서 두 칸 밖으로 나가서 찾으라고 명령!)
print("1. 남해안 정답지 데이터를 불러오고 가상 관측망(반경 2km)을 생성합니다.")
csv_path = '../../namhae_wide_marine_debris_windows (1).csv'

if not os.path.exists(csv_path):
    print(f"❌ 에러: '{csv_path}' 파일을 찾을 수 없습니다! 데이터가 yeosu 폴더에 있는지 확인해주세요.")
    exit()
...
# 2. MOHID 물리 데이터(VTU) 불러오기 (역시 ../../ 를 붙여서 두 칸 밖의 example 폴더를 찾음)
vtu_folder = '../../example' 
...
# 3. 결과 확인 및 저장 (결과물은 현재 폴더인 analysis 폴더 안에 예쁘게 저장됩니다)
print("\n✅ 분석 완료! AI가 학습할 '문제집' 결과입니다:")
final_dataset = gdf_truth[['지역명', 'mohid_particle_count', '수량(개)', '무게(kg)']]
print(final_dataset.head(10))

final_dataset.to_csv("AI_Training_Dataset.csv", index=False, encoding='utf-8-sig')
print("\n💾 'AI_Training_Dataset.csv' 파일로 저장이 완료되었습니다!")