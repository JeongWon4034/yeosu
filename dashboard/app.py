...
[변경사항]
- 다섬이 이미지 파일(dasomi_nobg.png) 연동 방식으로 변경 (누끼 이미지 적용)
- 사이드바 슬라이더, 체크박스 등 입력 컨트롤 컬러를 파란색으로 통일 (커스텀 CSS)
- 헤더 타이틀과 뱃지 문구 위치 스왑 (대시보드 목적 강조)
"""
...
# [수정] 배경이 제거된 다섬이 이미지(dasomi_nobg.png)를 읽어 Base64로 변환하는 함수
@st.cache_data
def get_local_image_base64(filepath):
    ...
    # 파일이 없을 경우 빈 이미지 또는 대체 텍스트 처리 방지용
    return ""
...
<style>
    ...
    /* =========================================================
       [수정] 사이드바 위젯(슬라이더, 체크박스 등) 파란색 테마 강제 적용
       ========================================================= */
    /* 슬라이더 채워진 영역 및 핸들 */
...
EXPO_MAIN    = {"돌산도", "금오도", "개도"}
EXPO_LINKED  = {"거문도", "낭도", "송도", "연도"}
OBS_KEYWORDS = ["백야도", "안도"]
...
with st.sidebar:
    ...
    if date_mode == "단일 날짜":
        ...
        date_range = st.date_input(
            "기간",
            value=(date_type(2026, 3, 28), date_type(2026, 4, 1)),
            label_visibility="collapsed",
        )
...
if sel_trash_types:
    mask = display_df["trash_types"].apply(
...
# [수정] 타이틀과 뱃지 텍스트 스왑 적용 & 다섬이 누끼 이미지 적용
st.markdown(f"""
<div class="expo-header">
  ...
  <svg class="expo-header-waves" viewBox="0 0 1200 48" preserveAspectRatio="none"
    ...
    <path d="M0,30 C150,48 350,8 600,28 C850,48 1050,10 1200,26 L1200,48 L0,48 Z" fill="rgba(255,255,255,0.12)"/>
    <path d="M0,36 C200,18 400,44 600,34 C800,24 1000,42 1200,32 L1200,48 L0,48 Z" fill="rgba(255,255,255,0.18)"/>
    <path d="M0,42 C300,30 600,46 900,36 C1050,30 1150,40 1200,38 L1200,48 L0,48 Z" fill="rgba(255,255,255,0.25)"/>
  ...
  <div class="expo-header-content">
    ...
    <div style="display:flex;align-items:flex-end;gap:10px;flex-shrink:0;">
      <img src="{DASOMI_SRC}" style="
        height:120px;
        filter: drop-shadow(2px 4px 8px rgba(0,40,100,0.3));
        margin-bottom:-4px;
...
if not filtered_df.empty:
    ...
    if r1 >= 80:
        ...
        border_c, bg_c, label_c, label_txt = "#F97316", "#FFF8F0", "#EA580C", "주의 필요"
    else:
        border_c, bg_c, label_c, label_txt = "#EAB308", "#FEFCE8", "#CA8A04", "모니터링"
    st.markdown(f"""
<div style="
    background:{bg_c};
...
">
  ...
  <div style="flex:1;font-size:.88rem;color:#1A2B3C;">
    ...
    <span style="color:#4B6178;"> · 쓰레기 약 {int(t1["trash_cnt"])}개 · {t1["main_type"]}</span>
    <br>
...
with tab_map:
    if top_targets.empty:
        ...
        with info_col:
            st.markdown('<p class="sec">수거 순서 (위험도 내림차순)</p>',
                        unsafe_allow_html=True)
            for rank, row in top_targets.iterrows():
                r  = int(row["risk"])
                tc = int(row["trash_cnt"])
            ...
            for label, val in [("평균 위험도", f'{top_targets["risk"].mean():.1f}'),
                                ("총 수거 예상량", f'{tot:,}개'),
        ...
        with map_col:
            top_targets["color"] = top_targets["risk"].apply(
                lambda r: risk_color(int(r), 255))
            ...
            layers = [
                ...
                *([pdk.Layer("ScatterplotLayer",
                    display_df[display_df["has_obs"]],
                    get_position="[lon, lat]",
                    get_color=[237, 233, 254, 90],
                    get_radius=750, radius_min_pixels=12, radius_max_pixels=26,
                ...
                pdk.Layer("TextLayer",
                    ...
                    billboard=True)]
                   if not display_df[display_df["has_obs"]].empty else []),

                *([pdk.Layer("ScatterplotLayer",
                    display_df[display_df["is_expo"]],
                    get_position="[lon, lat]",
                    get_color=[220, 252, 231, 60],
                    get_radius=900, radius_min_pixels=14, radius_max_pixels=30,
                    stroked=True, line_width_min_pixels=3,
            ...
            st.pydeck_chart(pdk.Deck(
                ...
                tooltip={"text": "{tooltip}",
                         "style": {"backgroundColor": C["white"],
                                   "color": C["gray_dark"],
                                   "fontSize": "12px",
                                   "border": f"1px solid {C['gray_light']}",
                                   "borderRadius": "4px",
                                   "padding": "8px 10px"}},
            ...
            st.markdown(
                ...
                f'<span><span style="color:#7C3AED;font-weight:700;">◉</span> 해양관측소 (항상 표시)</span>'
                f'<span><span style="color:#16A34A;font-weight:700;">◉</span> 박람회 공식 행사장</span>'
                f'</div>', unsafe_allow_html=True)

# ──────────────────────────────────────────────
# TAB 2~5 유지 (생략 없이 동일하게 적용)
# ──────────────────────────────────────────────
with tab_detail:
          ...
          <table style="width:100%;font-size:.84em;border-collapse:collapse;">
            <tr><td style="padding:4px 0;color:{C['gray_mid']};">위험도</td>
                <td style="text-align:right;">
                  <b style="color:{risk_hex(r)};">{r} — {risk_label(r)}</b></td></tr>
            ...
            <tr><td style="padding:4px 0;color:{C['gray_mid']};">유무인도</td>
                <td style="text-align:right;">
                  <b>{"유인도" if row["inhabited"] else "무인도"}</b></td></tr>
            <tr><td style="padding:4px 0;color:{C['gray_mid']};">연육도현황</td>
                <td style="text-align:right;">
                  <b>{"연륙교/연도교 있음" if row["has_bridge"] else "없음 (선박 수거)"}</b></td></tr>
    ...
    with c2:
        ...
        <div class="detail-card">
          ...
          <table style="width:100%;font-size:.84em;border-collapse:collapse;">
            {"".join(f'<tr><td style="padding:4px 0;color:{C["gray_mid"]};">{k}</td>'
    ...
    with c3:
        st.markdown(f'<div class="detail-card">'
                    ...
                    f'letter-spacing:.08em;color:{C["gray_mid"]};margin-bottom:8px;">'
                    f'5일 위험도 추이 (LSTM)</div>',
                    unsafe_allow_html=True)
...
with tab_predict:
    if top_targets.empty:
        ...
        sel2 = st.selectbox("분석 대상 섬",
                            top_targets["name"].tolist(), index=0,
                            key="predict_sel")
        row2 = top_targets[top_targets["name"] == sel2].iloc[0]
        st.plotly_chart(
            build_forecast_chart(sel2, int(row2["risk"]),
                                 int(row2["trash_cnt"]), predict_date),
            use_container_width=True)
        ...
        for i, (_, row) in enumerate(top_targets.head(3).iterrows()):
            ...
            with cols[i]:
                st.markdown(
                    ...
                    f'<div style="font-size:.7rem;text-transform:uppercase;'
                    f'letter-spacing:.08em;color:{C["gray_mid"]};margin-bottom:6px;">'
                    f'{risk_label(r)}</div>'
...
with tab_data:
    ...
    with dl1:
        ...
        st.dataframe(
            filtered_df[[
                "name","lat","lon","risk","trash_cnt","trash_types",
        ...
        csv1 = filtered_df.to_csv(index=False, encoding="utf-8-sig")
        st.download_button(
            label="CSV 다운로드 (필터 적용)",
            data=csv1,
            file_name=f"yeosu_filtered_{predict_date}.csv",
            mime="text/csv",
        )
    with dl2:
        ...
        if not top_targets.empty:
            ...
            csv2 = top_targets.to_csv(index=False, encoding="utf-8-sig")
            st.download_button(
                label="CSV 다운로드 (수거 대상)",
                data=csv2,
                file_name=f"yeosu_targets_{predict_date}.csv",
                mime="text/csv",
            )
        else:
            st.info("수거 대상 섬이 없습니다.")
    ...
    st.markdown("---")
    st.markdown("**전체 356개 섬 원본 데이터**")
    csv3 = islands_df.to_csv(index=False, encoding="utf-8-sig")
    st.download_button(
        label="CSV 다운로드 (전체 데이터)",
        data=csv3,
        file_name=f"yeosu_all_{predict_date}.csv",
        mime="text/csv",
    )
    st.dataframe(
